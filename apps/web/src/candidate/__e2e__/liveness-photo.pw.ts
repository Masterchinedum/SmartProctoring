 ...
 *     captures step frames; after the no-progress timeout the attempt goes to the server, which
 *     records a failed live-person check (retry, counted toward the attempt limit).
 ...
 *     with the attempt limit (2) reached the exam is put on hold for review.
 */
...
test('active liveness: a still photo does not pass', async () => {
  ...
  try {
    ...
    // … so after the no-progress timeout the attempt is handed to the server, which records the failed
    // live-person check (counted toward the attempt limit) and explains what to do.
    await expect(page.getByTestId('check-retry')).toBeVisible({ timeout: 60_000 });
    await expect(page.getByTestId('check-retry')).toContainText('Attempts remaining: 1');
    await expect(page.getByTestId('check-retry-guidance')).toContainText('Turn your whole head');
    ...
    console.log(
      ...
      JSON.stringify({
        ...
        reasons: second.liveness?.reasons,
        steps: second.liveness?.steps.map((st) => `${st.action}:${st.passed ? 'ok' : 'fail'}(${st.measured ?? '-'})`),
        stepFrames: second.frames.filter((f) => f.step !== 'frontal').map((f) => `${f.step}:${f.stepSatisfied ? 'ok' : 'no'}`),
      }),
    );
    expect(second.frames.filter((f) => f.step === 'frontal').every((f) => f.status === 200)).toBe(true);
    expect(second.outcome).not.toBe('passed');
    expect(second.liveness?.passed).toBe(false);

    // After the configured attempts (2) the exam is held for human review — never 'ready'.
