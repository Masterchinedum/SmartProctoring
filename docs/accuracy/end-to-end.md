...
### Balanced sampling intensity (scenarios 21 and 22)

The per-exam *Balanced* sampling intensity (`identity.samplingProfile`, ARCHITECTURE §4): routine identity samples
of 2 frames every 12 s for the first 3 minutes, then every 30 s; samples triggered by a face change, the exam start
and server requests keep 3 frames. It serves about 2× the candidates per server at a synchronised start and 2.5× in
steady state (PERFORMANCE §7.7). Measured with the same realistic swap and long genuine scenarios
(`E2E_SAMPLING=balanced`):

<!-- rw:tables:balanced:start (generated by `pnpm --filter @sp/e2e rw:report -- --run <id> --write --section balanced`) -->

Measurement runs: 2026-09-25T10-19-53-716Z — 27 measurements, 2026-09-25 10:20 to 2026-09-25 10:44 UTC; commit f72c1ab+dirty; 2 Playwright worker(s) on 4 CPUs; machine load (1-min load average at the end of each test) min 0.6 / median 0.9 / max 2.2; identity sampling profile balanced.

### Summary against the targets

| requirement / target | runs | met | key numbers (median / max) | verdict |
|---|---|---|---|---|
| Genuine candidate never called a different person (all resume + long runs) | 6 | 0 false identity_mismatch |  | **met** |
| Quick swap right after exam start (typical light; gap / cross-dissolve / slide; 720p / 480p): held as identity_mismatch, median ≲ 20 s | 12 | 12/12 | delay new person in view → hold 4.9 / 10.9 s | **met** |
| Swap in a dim room (no gap, 480p): held, or staff-visible suspect / inconclusive (non-matching identity check or identity event) within ~30 s | 3 | 3/3 | held 0/3, delay – / – s; first signal 1.4 / 1.4 s | **met** |
| Family member (father replaces son) mid-exam: held, or staff-visible suspect / inconclusive within ~30 s | 6 | 6/6 | held 6/6, delay 4.9 / 5 s; first signal 0.6 / 0.8 s | **met** |
| Genuine candidate ≥ 5 min with lighting changes and head movement: zero identity_mismatch, no hold | 6 | 6/6 | 121 samples; identity_unverifiable 0 | **met** |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| gap-720p [balanced] | 1280x720 | 3 | hold: 3/3 | 3/3 | 4.9 / 5.7 | 7.5 / 8.3 | 4.7 | 0.5 / 0.5 | 3/3 | 0/3 | +0.5s track_break:inconclusive@0.36 +3.9s appearance_change:inconclusive@0.39; +0.5s track_break:inconclusive@0.34 +3.9s server_request:inconclusive@0.38; +0.5s track_break:inconclusive@0.36 +3.9s server_request:inconclusive@0.39 +4.7s appearance_change:inconclusive@0.38 |
| no-gap-crossfade-720p [balanced] | 1280x720 | 3 | hold: 3/3 | 3/3 | 10.9 / 10.9 | 11.4 / 11.4 | 4.8 | 0.6 / 0.6 | 3/3 | 0/3 | +0.6s appearance_change:inconclusive@0.38 +6.6s server_request:inconclusive@0.37 +10s server_request:inconclusive@0.35; +0.6s appearance_change:inconclusive@0.35 +4s server_request:inconclusive@0.35; +0.6s appearance_change:inconclusive@0.38 +6.7s server_request:inconclusive@0.37 +9.9s server_request:inconclusive@0.34 |
| gap-480p [balanced] | 640x480 | 3 | hold: 3/3 | 3/3 | 4.7 / 5.6 | 7.3 / 8.2 | 4.8 | 0.5 / 0.5 | 3/3 | 0/3 | +0.5s track_break:inconclusive@0.36 +3.9s server_request:inconclusive@0.35; +0.4s track_break:inconclusive@0.36 +3.8s server_request:inconclusive@0.36; +0.5s track_break:inconclusive@0.37 +3.7s server_request:inconclusive@0.37 +4.4s appearance_change:inconclusive@0.36 |
| no-gap-slide-480p [balanced] | 640x480 | 3 | hold: 3/3 | 3/3 | 4.8 / 4.9 | 5.3 / 5.4 | 4.8 | 0.6 / 0.8 | 3/3 | 0/3 | +0.6s appearance_change:inconclusive@0.32 +4s server_request:inconclusive@0.32; +0.6s appearance_change:inconclusive@0.32 +4s server_request:inconclusive@0.32; +0.8s appearance_change:inconclusive@0.36 +4s server_request:inconclusive@0.36 |
| dim-no-gap-crossfade-480p [balanced] | 640x480 | 3 | signal: 3/3 | 0/3 | – / – | – / – | 4.8 | 1.4 / 1.4 | 3/3 | 0/3 | +1.4s appearance_change:inconclusive@0.38 +5s server_request:inconclusive@0.32 +8.4s server_request:inconclusive@0.45 +11.5s server_request:inconclusive@0.40; +1.4s appearance_change:inconclusive@0.37 +5s server_request:inconclusive@0.30 +8.4s server_request:inconclusive@0.45 +11.7s server_request:inconclusive@0.29; +1.4s appearance_change:inconclusive@0.55 +7.2s server_request:inconclusive@0.59 +11.3s server_request:inconclusive@0.58 +14.7s server_request:inconclusive@0.46 |

### Family member takes over mid-exam (scenarios 21 / 3)

| variant | camera | runs | requirement met | held (identity_mismatch) | delay s: new person in view → hold, med/max | delay s: transition start → hold | exam start → swap s | first staff-visible signal s | faster sampling requested | false alarm before swap | first identity checks after the swap (s after new person in view) |
|---|---|---|---|---|---|---|---|---|---|---|---|
| family-gap-720p [balanced] | 1280x720 | 3 | signal: 3/3 | 3/3 | 4.7 / 4.9 | 7.3 / 7.5 | 4.7 | 0.5 / 0.5 | 3/3 | 0/3 | +0.5s track_break:inconclusive@0.32 +3.9s server_request:inconclusive@0.33; +0.5s track_break:inconclusive@0.30 +3.9s server_request:inconclusive@0.31; +0.5s track_break:inconclusive@0.32 +3.9s server_request:inconclusive@0.32 |
| family-no-gap-crossfade-480p [balanced] | 640x480 | 3 | signal: 3/3 | 3/3 | 4.9 / 5 | 5.4 / 5.5 | 4.7 | 0.8 / 0.8 | 3/3 | 0/3 | +0.6s appearance_change:mismatch@0.30 +4.2s server_request:inconclusive@0.31; +0.8s appearance_change:mismatch@0.28 +4s server_request:inconclusive@0.31; +0.8s appearance_change:inconclusive@0.31 +4s server_request:inconclusive@0.32 |

### Genuine candidate, long runs (scenario 22)

| case | run | camera | minutes | identity samples | decisions | min similarity | median similarity | identity_mismatch | identity_unverifiable | lighting_unusable | held | faster samples requested |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| typical-with-light-changes [balanced] | 1 | 1280x720 | 5 | 21 | match 21 | 0.78 | 0.97 | 0 | 0 | 0 | no | 0 |
| typical-with-light-changes [balanced] | 2 | 1280x720 | 5 | 22 | match 22 | 0.79 | 0.97 | 0 | 0 | 0 | no | 0 |
| typical-with-light-changes [balanced] | 3 | 1280x720 | 5 | 21 | match 21 | 0.77 | 0.97 | 0 | 0 | 0 | no | 0 |
| mostly-dim [balanced] | 1 | 1280x720 | 5 | 19 | match 19 | 0.73 | 0.82 | 0 | 0 | 0 | no | 0 |
| mostly-dim [balanced] | 2 | 1280x720 | 5 | 19 | match 19 | 0.75 | 0.81 | 0 | 0 | 0 | no | 0 |
| mostly-dim [balanced] | 3 | 1280x720 | 5 | 19 | match 19 | 0.74 | 0.81 | 0 | 0 | 0 | no | 0 |

### Runs that missed their target

None.

<!-- rw:tables:balanced:end -->

**Balanced against Maximum accuracy.** The Maximum figures are from the run in the tables above.

| | Maximum accuracy | Balanced |
|---|---|---|
| quick swaps held | 12/12 | 12/12 |
| quick swaps: new person in view → hold, median / max | 5.0 / 11.1 s | 4.9 / 10.9 s |
| family swaps held | 6/6 (4.9 / 5.0 s) | 6/6 (4.9 / 5.0 s) |
| dim-room look-alike: staff-visible signal (not held) | 3/3 (1.4 / 1.5 s) | 3/3 (1.4 / 1.4 s) |
| genuine 5-min runs: false alarms | 0 | 0 |
| genuine 5-min runs: identity samples | 213 (6 runs) | 121 (6 runs) |
| genuine 5-min runs: faster samples requested | 0 | 0 |

Every swap in these scenarios happens at a trigger: the face track breaks or its appearance changes. The swap is
then confirmed by the 3-frame follow-ups the server asks for, and Balanced changes neither. What Balanced does change
is how soon a swap with *no* trigger is confirmed: about 8 s later at the median and 14 s later at p90 in the
simulation (`identity-v2.md` §6.4, "Sampling intensity"). The genuine runs never left `consistent`, so they took no
faster follow-ups.
