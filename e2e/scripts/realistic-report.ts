 ...
 *   pnpm --filter @sp/e2e rw:report -- --run <id> --write --section balanced
 *                                                            # ... between the rw:tables:balanced markers instead
 *
 * Runs with a non-default identity sampling profile (E2E_SAMPLING=balanced, recorded as `samplingProfile`) appear as
 * their own rows ("case [balanced]").
 */
...
const recs = all
  .filter((r) => !runs || runs.has(r.runId))
  .map((r) => (r.samplingProfile && r.samplingProfile !== 'maximum' ? { ...r, case: `${r.case} [${r.samplingProfile}]` } : r));
...
out.push(
  `Measurement runs: ${[...new Set(recs.map((r) => r.runId))].join(', ')} — ${recs.length} measurements, ` +
    ...
    `min ${f1(min(loads))} / median ${f1(med(loads))} / max ${f1(max(loads))}; identity sampling profile ` +
    `${[...new Set(recs.map((r) => r.samplingProfile ?? 'maximum'))].join(', ')}.\n`,
);
...
if (args.includes('--write')) {
  ...
  const section = args.includes('--section') ? `:${args[args.indexOf('--section') + 1]}` : '';
  const start = `<!-- rw:tables${section}:start (generated by \`pnpm --filter @sp/e2e rw:report -- --run <id> --write${section ? ` --section ${section.slice(1)}` : ''}\`) -->`;
  const end = `<!-- rw:tables${section}:end -->`;
  const i = cur.indexOf(`<!-- rw:tables${section}:start`);
